...
def game_loop(settings):
    ...
    ended = False
    player = None
    state_reader = None # Something that combines the player and generator data into a state
    ...
    while tick < settings.max_ticks and not ended:
        ...
        settings.agent.record(state=state_reader.get_state(), action=None, reward=reward_function(state_reader, generator))
